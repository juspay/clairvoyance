...
| `line-nudge-drain.json` | The lending journey with the **overnight drain** (21 Sep 2026; `../runbooks/overnight-drain.md`) | A run whose timer a window held overnight is `cold`; hot runs are claimed first, cold runs oldest-held-first, and a cold run's call is written only while its template's number has a free line (the insert itself checks queued + on the line against `maximum_channels`); a cold run turns hot the moment its call is queued. Each call square says `"await": true` and waits for its own call's report (`await_minutes` the backstop) before its plain edge, so a run never has two calls queued and `gap-30m` counts from the call's end; the call squares list the merchant topics with arrows back to the rule, so a letter while a call is queued aborts that call and re-decides. A call to a phone that cannot be dialled or a blacklisted customer ends the run as `ejected` |
