...
| `line-nudge.json` | A lending journey on a merchant's own registered events, with a `condition` on a fact rendered from inside an array and a `merchant_http` action whose answer the call reads (enh A/03) | LINE_INITIATED → **is-credit-line**: `context.offers exists` → quiet 30m (every non-terminal event re-arms it and re-checks) → call → listen 30d; `else` → listen. `offers` exists only when the topic's `item_where` kept at least one credit line with an offer, and then carries only those lines, numbered, each with its lender and offer id |
