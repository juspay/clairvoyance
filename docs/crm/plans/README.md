...
| `cart-recovery-split.json` | The cart board with a `split` square (enh A/04) | wait 30m → **which-letter**: 70% `control` → `cart_recovery_1`, 30% `variant` → `cart_recovery_2` → wait 1d. One square sends a fixed share of runs down each arm and remembers which arm each run took, so the plan's summary reports each arm's runs, open runs and exits by reason (`by_split`) — which letter recovered more carts, not only that the split was honest. The arm is a hash of the run id and the square id, so a lease retry never moves a run between arms; the shares total 100 because a split has no `else` |
