...
| `product-aware-call.json` | One call square choosing its TEMPLATE by condition (`template_rules`) | `LINE_INITIATED` / `LINE_OFFERED` → quiet 30m within the calling window → **nudge-call**, which reads an if/else-if ladder over facts in hand: `event_name is LINE_INITIATED` AND `category exists` → the category letter; `event_name is LINE_INITIATED` AND `products not_exists`, OR `event_name is LINE_OFFERED` AND `brand not_exists` → the no-product letter; anything else → the square's own `template_id`. The square does NOT branch — one arrow out, one lead, one visit counter — so only the cargo differs. Conditions within one arm are ANDed and OR is two arms naming the same template, because a predicate has no `or`. The default is mandatory and is the `else`: no arm holding fires it rather than parking the run |
