...
| `line-nudge-offset.json` | The lending journey with the **morning offset** (22 Sep 2026; `../runbooks/morning-offset.md`) | `offset_minutes: 90` on the timers' window reserves the first 90 minutes after the opening for the runs the window held overnight: they wake at 09:00 and their calls go to the dialler at once; every timer set inside those minutes runs 90 minutes longer, so live customers queue behind the pile, not beside it. Each call square says `"await": true` and waits for its own call's report (`await_minutes` the backstop) before its plain edge; the call squares list the merchant topics with arrows back to the rule, so a letter while a call is queued aborts that call and re-decides. A call to a phone that cannot be dialled or a blacklisted customer ends the run as `ejected` |
