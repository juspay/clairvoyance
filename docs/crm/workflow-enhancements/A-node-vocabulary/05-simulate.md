# A/05 — Dry run: simulate a plan against a sample event (G10)

**Track A · step 5** · **Kind**: feat · **PR title**: `feat(crm): POST /workflows/{id}/simulate — walk a document with a fake clock and no writes (enh A/05)` · **Depends on**: A/01, A/02, A/04 (rollout 17 ladder is merged) · **Notes**: §16.3 G10
