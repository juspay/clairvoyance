# Phase 22 — `http` action node (G5 part 2)

**Kind**: feat · **PR title**: `feat(crm): http action node — call a merchant endpoint from a run, results become facts` · **Depends on**: 15, 16, 20; **PR #987 merged** (`fix(security): SSRF egress guard on every server-side fetch`, murdore) — if not merged, stop: an outbound-fetch node without the egress guard is a hole, not a feature. · **Notes**: §16.3 G5 · **Wave 6**
...
- Execution: `execute_http` via `app/core/transport/http_client.create_http_client` behind the #987 egress guard; success → `facts_from_response` paths (same tiny grammar as phase 21's `extract_letter_fact`) merged into context as `http_<node>_<key>`… decision: merged under `facts.<node>` (phase 16 namespacing) and mirrored flat for templates by `run_facts` precedence. Classification: transport error / 5xx / 429 → raise (transient; the walker's retry ladder handles it); 4xx or unexpected status → `NodeParked` with status and a 200-char body excerpt masked by `mask_digit_runs`.
