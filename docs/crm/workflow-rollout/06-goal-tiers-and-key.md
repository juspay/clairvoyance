...
- `walker._advance`: iterate tiers; call `customer_has_event(..., since, where=(tier.key.event, run.context.get(tier.key.run)) if tier.key else None)`; exit with the tier's reason. An unkeyed tier's `where` is no longer bare `None` by default: when every door of the plan agrees on one `entry.key` (`WorkflowDefinition.entry_key()`), that key and `run.enrollment_key` fill it — two applications on one identity must not fire one run's fire-time exit off the other's terminal event. Identity-wide stays the fallback only when the plan has no key at all (cart-recovery-style), the tier's own topics touch `ATTRIBUTED_TOPICS` (`message.inbound` — a provider's own reply/outcome shape, e.g. a WhatsApp wamid, never a fact about the run), or the key does not resolve to a flat top-level payload field (`record/catalog.py::flat_payload_field`; `customer_has_event_query`'s `where` is a literal `payload->>$n`, so a nested/derived key like `flow_token` can never match it and must not silently narrow to nothing). `ATTRIBUTED_TOPICS` names only the topics to EXCLUDE from the fallback, never the ones to require — a goal tier is almost always on a different, downstream topic from the door it started on, so restricting the fallback to literal door topics would defeat the ordinary case it exists for. Same rule `entry.py:_scoped_to_this_run` applies to the event-consumption path; one set, both callers agree.
