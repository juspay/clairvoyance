...
5. **Old versions are kept for the life of the plan — never deleted.** *(Amended 2026-09-03 in rollout phase 14. The original point read: "retained while any non-exited run references them, then swept by the walker pod's housekeeping after a retention window; the latest version is never swept." The sweep was designed, built and dropped before merge: a version row is one small document, a plan publishes tens of them, every read is a point lookup on the unique index, and an exited run's `workflow_version` — the audit fact this record introduces — must keep answering "what did this run execute". Migration 064's comment that omits a DELETE guard "because the retention sweep deletes unreferenced old versions" is superseded by this point; a DELETE-refusing trigger is a candidate for a later migration.)*
6. **A template a retained version names may not be retired while runs reference that version** — nor while a live or paused plan's latest document names it, since its next entrant would be pinned to the withdrawn template the same way. Retire refuses with both counts. The check and the local withdrawal share one transaction, ahead of the provider call, under a transaction-scoped Postgres advisory lock keyed by the template (`app/crm/shared/locks.py`): every pinning path — enrol, publish, migrate-forward — holds that key SHARED for each template the document it pins sends, and retirement holds it EXCLUSIVE, so retirement waits for in-flight pinners and counts their rows, later pinners wait for its verdict, and pinners never block one another. The guard's count is registered from the composition root (the record/consumers.py inversion) so connectivity never imports outreach.
...
- **Tooling:** migrate-forward (move waiting runs from vN to vN+1 when their current node still exists and `entry` is unchanged — the stranding validator reused as a pure function), a per-version run count, the template-retirement guard (phase 14). No retention sweep — see point 5.
