...
## 18. Widget Public Mode (Unified Chat ↔ Voice)

> **Prereq:** PostgreSQL ≥ 11. Migration 030 uses `ADD COLUMN NOT NULL
> DEFAULT 'CHAT'` on `chat_session`, which is a metadata-only operation
> from pg 11 onward (no table rewrite). Older PG versions would rewrite
> the entire table under `ACCESS EXCLUSIVE`; deploy targets must pin
> pg ≥ 11 before applying this migration.
...
### 18.1 What it solves vs `/chat/demo` and `/demo/connect`
...
### 18.3 State machine on `current_channel`
...
### 18.6 widget_config (per-merchant)

Created by migration 030 (along with the chat_session columns).
...
### 18.7 Voice runtime — resume + drain
...
### 18.8 Manual verification checklist (no automated tests in this PR)

1. `python scripts/migrate.py up` — 030 applies cleanly (creates
   `widget_config` table + adds `chat_session.current_channel` and
   `chat_session.voice_lead_id`).
...
- `app/database/migrations/029_widget_public_mode.sql` (creates
  `widget_config` table + adds `chat_session.current_channel` and
  `chat_session.voice_lead_id` — both ship together because each is
