...
- **Reseller with `["*"]`**: Access to ALL merchants in the system
...
async def resolve_merchant_ids(user: UserInfo) -> Optional[List[str]]:
    ...
    - Reseller with ["*"] → None (all merchants)
    - Reseller with specific IDs → those IDs
...
| Reseller | `["*"]` | Unrestricted — all merchants |
| Reseller | `["redbus", "uber"]` | Only those two |
...
# Returns None (unrestricted) or List[str] (specific IDs)
```
...
# Returns None (unrestricted) or a specific list
# Admin → always None
# Reseller with ["*"] → None
