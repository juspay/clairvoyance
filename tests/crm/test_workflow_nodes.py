...
def test_a_wait_listens_and_branches_exactly_when_it_lists_topics() -> None:
    ...
    # … except the call that waits for its report (22 Sep 2026), which
    # listens for that one event; without `event_name` it is the old
    # square, deaf.
    ringing = WorkflowNode(id="c", type="call", event_name="call.completed")
    assert ringing.topics == ["call.completed"] and ringing.key == "$topic"
    assert (listens(ringing), branches(ringing)) == (True, True)
    deaf = WorkflowNode(id="c", type="call")
