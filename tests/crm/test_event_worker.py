...
def test_pass_returns_claimed_rows_even_when_every_row_failed(
    ...
    async def fake_process_one(conn: Any, event: RawEvent, *rest: Any) -> None:
...
def _pass_over(
    ...
    """run_pass with the atom faked: the body runs over a fake txn, and
    run_pass's after-commit step (marking discovered topics) is real."""

    async def fake_claim(conn: Any, limit: int) -> List[RawEvent]:
        return list(events)

    async def fake_atomically(fn: Any, *args: Any) -> Any:
        return await fn(_fake_txn(), *args)

    monkeypatch.setattr(
        fake_accessor, "claim_pending_events", fake_claim, raising=False
    )
    monkeypatch.setattr(workers, "atomically", fake_atomically)
    _run(workers.run_pass(10))
...
def test_discovery_writes_the_nudge_row_once_per_topic_ever(
    ...
    # three of one topic and one of another in one pass; then a second pass
    _pass_over(
        monkeypatch,
        fake_accessor,
        [_event(event_id=f"evt-{i}") for i in range(3)]
        + [_event(event_id="evt-x", topic="lead.updated")],
    )
    _pass_over(monkeypatch, fake_accessor, [_event(event_id="evt-later")])

    assert fake_accessor.detected == [
        ("m1", "lead-api", "lead.pushed"),
        ("m1", "lead-api", "lead.updated"),
    ]
    assert catalog.is_known(("m1", "lead-api", "lead.pushed"))
    assert catalog.is_known(("m1", "lead-api", "lead.updated"))


def test_a_rolled_back_nudge_is_not_remembered_as_written(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The nudge INSERT shares the row's savepoint: a consumer raise rolls
    it back. Marking the topic known at INSERT time would then hide the
    loss until a restart; marking after the commit — and only rows that
    survived — means the next pass writes it again."""
    fake_accessor = _FakeAccessor()
    monkeypatch.setattr(workers, "accessor", fake_accessor)
    ...
    async def poison(event: RawEvent, *rest: Any) -> None:
        if blow_up["on"]:
            raise RuntimeError("workflow rule blew up")

    monkeypatch.setattr(workers, "_consume_attributed_event", poison)

    _pass_over(monkeypatch, fake_accessor, [_event(event_id="evt-1")])
    key = ("m1", "lead-api", "lead.pushed")
    assert fake_accessor.detected == [key], "the INSERT ran (and rolled back)"
    assert not catalog.is_known(key), "...so it must not be remembered"

    blow_up["on"] = False
    _pass_over(monkeypatch, fake_accessor, [_event(event_id="evt-1", attempts=2)])
    assert fake_accessor.detected == [key, key], "the next pass writes it again"
    assert catalog.is_known(key)


def test_the_processor_alone_never_marks_a_topic_known(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """_process_one returns the key it nudged; only run_pass, after the
    commit, marks it. (A code-layer topic writes nothing and is marked at
    once — test_discovery_never_nudges_for_a_code_layer_topic.)"""
    fake_accessor = _FakeAccessor()
    monkeypatch.setattr(workers, "accessor", fake_accessor)
    ...
    monkeypatch.setattr(workers, "crm_resolve", fake_resolve)
    key = _run(workers._process_one(_fake_txn(), _event()))
    assert key == ("m1", "lead-api", "lead.pushed")
    assert not catalog.is_known(key)
