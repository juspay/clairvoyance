...
def test_lookup_shares_the_validator_s_whitelist() -> None:
    """The executor reads only the five columns the validator admits. A
    field the grammar would refuse — customer.attributes whole, a model
    method — is absent here too, whether or not a document ever spelled
    it: defence in depth, one whitelist."""
    customer = CustomerFacts(
        display_name="Priya", has_phone=True, attributes={"vip": "yes"}
    )
    assert predicates.lookup("customer.display_name", {}, {}, customer) == "Priya"
    assert predicates.lookup("customer.attributes.vip", {}, {}, customer) == "yes"
    assert predicates.lookup("customer.attributes", {}, {}, customer) is None
    assert predicates.lookup("customer.model_dump", {}, {}, customer) is None
    assert predicates.lookup("customer.attributes.phone", {}, {}, customer) is None


def test_an_incomplete_persisted_claim_is_ignored_not_fatal() -> None:
    """A claim bucket written before the evidence field existed (or by a
    hand edit) has `at` and no `e`. The winner ladder reads c["e"]; such a
    claim must fall out before the ladder, so the predicate resolves to
    else instead of the condition square raising."""
    attributes = {
        "tier": [
            {"v": "gold", "at": "2026-09-01T00:00:00Z"},  # no evidence: ignored
            {"v": "silver", "e": "observed", "at": "2026-09-02T00:00:00Z"},
        ],
