...
def test_the_route_declares_the_tenancy_door_and_is_mounted() -> None:
    """The dry run reads a plan, so it stands behind the same door as every
    other workflows route (ADR 0007 amendment, f967e0b3): merchant_scope
    finds the merchant and checks the caller, and the user dependency
    rides along for the author stamp."""
    route = next(
        r
    ...
    def walk(dependant, name: str) -> bool:
        for d in dependant.dependencies:
            call = getattr(d, "call", None)
            qual = getattr(call, "__qualname__", "") if call is not None else ""
            if qual.startswith(name) or getattr(call, "__name__", "") == name:
                return True
            if walk(d, name):
                return True
        return False

    assert walk(route.dependant, "merchant_scope"), "the dry run needs the tenancy door"
    assert walk(route.dependant, "get_current_user_with_rbac")
