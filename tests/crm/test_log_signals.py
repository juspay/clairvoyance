...
def test_a_deduped_letter_is_counted_but_marked(
    ...
    # Its own words: a line that says "accepted" for a letter that was
    # not filed reads as a lie in a raw tail, whatever the field says.
    fields = lines.fields("duplicate event ignored")
...
# --- the ids a square hands to a dispatcher (review 5266010719 §1) ---


def test_the_call_square_binds_the_lead_id(monkeypatch: pytest.MonkeyPatch) -> None:
    """The 19 Sep incident: lead inserted 13:05, dialled 13:28 — visible
    only by joining this line to the Buddy worker's dial line ON LEAD ID.
    A join needs a column; in message text the id is unreachable."""
    import app.crm.outreach.nodes.call as call_node
    from tests.crm.test_workflow_call_visits import (
        _DEFINITION as CALL_DEF,
        _NODE as CALL_NODE,
        _install,
        _run as call_run,
    )
    ...
    lines = _Lines()
    monkeypatch.setattr(call_node, "logger", lines)
    inserted: List[str] = []
    _install(monkeypatch, inserted)

    asyncio.run(call_node.execute(call_run(), CALL_NODE, CALL_DEF))

    assert lines.fields("pushed lead")["lead_id"] == inserted[0]


def test_the_send_square_binds_the_message_id(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Same law as the call square: the id that joins this line to
    dispatch's terminal line rides as a field."""
    import app.crm.outreach.nodes.send as send_node

    lines = _Lines()
    monkeypatch.setattr(send_node, "logger", lines)

    async def queued(**kwargs: Any) -> str:
        return "msg-1"

    async def no_blocks(run: Any, node: Any, definition: Any, names: Any) -> Any:
        return {}, None

    monkeypatch.setattr(send_node, "queue_message", queued)
    monkeypatch.setattr(send_node, "blocks_for", no_blocks)
    node = WorkflowNode(
        id="nudge-wa", type="send", channel="whatsapp", template="tpl-1"
    )
    definition = SimpleNamespace(purpose_key="cart_recovery")
    run = _run()
    run.context["phone"] = "+919110752252"

    asyncio.run(send_node.execute(run, cast(Any, node), cast(Any, definition)))

    assert lines.fields("queued message")["message_id"] == "msg-1"


def test_the_action_square_logs_its_performance(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The fourth verb had NO line at all: a performed action was visible
    only on the run's context, which no rule reads."""
    import app.crm.outreach.nodes.action as action_node

    lines = _Lines()
    monkeypatch.setattr(action_node, "logger", lines)

    async def performed(*args: Any, **kwargs: Any) -> Dict[str, Any]:
        return {"status": "done"}

    async def no_blocks(run: Any, node: Any, definition: Any, names: Any) -> Any:
        return {}, None

    monkeypatch.setattr(action_node, "perform_action", performed)
    monkeypatch.setattr(action_node, "blocks_for", no_blocks)
    node = WorkflowNode(
        id="tag-1", type="action", connector="shopify", action="add_tag"
    )

    asyncio.run(
        action_node.execute(_run(), cast(Any, node), cast(Any, SimpleNamespace()))
    )

    fields = lines.fields("performed")
    assert fields["connector"] == "shopify"
    assert fields["action"] == "add_tag"
    assert fields["ok"] is True


# --- the REAL sink delivers both mechanisms (review 5266010719 §4) ---


def test_the_real_sink_lands_context_and_bind_as_columns(
    capsys: pytest.CaptureFixture,
) -> None:
    """_Lines pins names; this pins DELIVERY, through the real patcher and
    the real json_sink. Three behaviours a rule depends on: a context field
    becomes a top-level key, a bind field beats a colliding context field
    (the walker's per-square node refresh relies on it), and a None bind
    field survives as null while a None context field is dropped."""
    import json as jsonlib

    from app.core.logger import json_sink, log_context_patcher
    from app.core.logger.context import set_log_context

    set_log_context(merchant_id="m1", node="from-context", empty_ctx=None)
    record: Dict[str, Any] = {
        "time": NOW,
        "level": SimpleNamespace(name="INFO"),
        "name": "app.crm.outreach.walker",
        "function": "walk_run",
        "line": 1,
        "message": "walker pass",
        "module": "walker",
        ...
        "extra": {"node": "from-bind", "event_id": None},
    }
    log_context_patcher(record)
    json_sink(SimpleNamespace(record=record))

    line = jsonlib.loads(capsys.readouterr().out)
    assert line["merchant_id"] == "m1"  # context -> column
    assert line["node"] == "from-bind"  # bind wins a collision
    assert line["event_id"] is None  # None bind survives as null
    assert "empty_ctx" not in line  # None context is dropped
