...
@pytest.mark.asyncio
async def test_the_arm_answers_the_edge_and_survives_for_the_report() -> None:
    ...
    on the row for the report (read in SQL), because the reply is cleared
    when the token leaves. Bookkeeping, both: neither reaches a template."""
    ...
    # Ours to write, never a template's: the report groups on the row's
    # context in SQL; run_facts filters the arm out exactly like reply_.
    assert split_key("experiment") not in run_facts(written, node)
...
# --- the arm is bookkeeping: ours to write, never a producer's, never a template's


def test_the_arm_never_reaches_a_template_or_a_lead_payload() -> None:
    """run_facts is the ONE filter the call payload and the send variables
    derive from; the recorded arm must fall out of it like reply_ does, or
    it rides to buddy and on to the merchant's outcome webhook."""
    assert is_bookkeeping(split_key("which-letter"))
    facts = run_facts(
        {
            "order_id": "A1",
            reply_key("which-letter"): "control",
            split_key("which-letter"): "control",
        }
    )
    assert facts == {"order_id": "A1"}


def test_a_producer_cannot_spell_an_arm() -> None:
    ...
    assert context == {"order_id": "A1"}


def test_a_producer_key_cannot_invent_an_experiment_in_the_report() -> None:
    """workflow_split_counts_query reads every context key LIKE 'split_%'
    straight off the row. The only way a producer's `split_payment` could
