...
that across redirects since it follows them in one request() call. Because
``ssrf_safe_request`` follows them itself, each hop would otherwise start a
fresh N-second clock, and the remote server (not the caller) picks the hop
count.

Every test here uses a server whose hops are individually fast enough but
collectively too slow — a per-hop clock never fires on such a chain, only a
shared deadline does.
"""
...
async def test_the_caller_s_total_covers_the_whole_chain_not_each_hop():
    # 4 hops x 0.4s = 1.6s of server time vs a 0.9s budget; no single hop exceeds it.
...
async def test_the_session_default_is_used_when_the_caller_passes_no_timeout():
    # Recording downloads pass no timeout; the session's budget must cover the chain.
    runner, url = await _chain(hops=3, per_hop_delay=0.4)
    try:
        timeout = aiohttp.ClientTimeout(total=0.9)
...
async def test_the_other_timeout_fields_survive_the_clamp():
    # Only `total` shrinks per hop; connect/sock_read/sock_connect stay the caller's.
