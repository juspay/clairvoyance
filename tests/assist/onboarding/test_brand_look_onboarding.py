...
def _reonboard(monkeypatch, get_widget_answers: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Re-onboard a merchant whose widget reads back each appearance in turn;
    returns the appearance saved."""
    default = _default_template()
    ...
    widgets = [
        _widget(existing.id).model_copy(update={"appearance": appearance})
        for appearance in get_widget_answers
    ]
    update = AsyncMock(return_value=widgets[0])
    monkeypatch.setattr(service, "detect_look", AsyncMock(return_value=_look()))
    monkeypatch.setattr(
        service,
        "get_widget_config_by_reseller_merchant",
        AsyncMock(side_effect=widgets),
    )
    ...
    return update.await_args.kwargs["appearance"]


def test_reonboarding_keeps_the_merchants_colour(monkeypatch):
    colour = {"primary_color": "#000000"}
    assert _reonboard(monkeypatch, [colour, colour]) == {
        "primary_color": "#000000",
        "header_logo_url": LOGO,
    }


def test_a_colour_changed_during_the_run_is_kept(monkeypatch):
    # The merchant saved a new colour while the site was being read.
    saved = _reonboard(
        monkeypatch, [{"primary_color": "#000000"}, {"primary_color": "#123456"}]
    )
