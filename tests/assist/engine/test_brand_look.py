...
async def test_a_deadline_leaves_the_logo_its_share(monkeypatch):
    asked: List[float] = []

    async def render(url, *, timeout_seconds):
        asked.append(timeout_seconds)
        raise firecrawl.BrandLookUnavailable("slow")

    monkeypatch.setattr(firecrawl, "brand_look", render)
    await brand.resolve(
        "https://store.example/", _profile(), deadline=time.monotonic() + 30
    )

    assert len(asked) == 1
    assert 0 < asked[0] <= 30 - brand._LOGO_TIMEOUT_SECONDS


async def test_no_time_left_skips_the_render_and_keeps_the_platform_look(
    monkeypatch,
):
    rendered: List[str] = []
    ...
    look = await brand.resolve(
        "https://store.example/",
        _profile(),
        platform_look=brand.from_platform(RED, None),
        deadline=time.monotonic() + 1,
    )

    assert rendered == []
    assert look.color("primary") == RED
    assert "brand provider unavailable — skipped" in look.warnings
