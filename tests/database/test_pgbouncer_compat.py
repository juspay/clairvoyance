...
def _parse_or_fail(source: Path, root: Path) -> ast.Module:
    """Parse a file for a guard, or fail the guard.

    Skipping a file the scanner cannot parse reports it CLEAN -- the same
    fail-open hole the AST rewrite closed when it replaced substring matching.
    A bad create_pool() hiding in an unparsable file would pass. The whole
    tree parses today, so this never fires; if it starts firing, that is a red
    build asking for the file to be fixed, not a silent gap in the scan.
    """
    try:
        return ast.parse(source.read_text())
    except SyntaxError as exc:  # pragma: no cover - the tree parses today
        raise AssertionError(
            f"{source.relative_to(root)} does not parse ({exc}). The "
            "transaction-pooling guards cannot scan it, and must not silently "
            "skip it. Fix the file."
        ) from exc
...
def test_every_connection_site_disables_statement_cache() -> None:
    ...
    for source in sorted(
        ...
        tree = _parse_or_fail(source, root)
...
async def rig_pool(monkeypatch: pytest.MonkeyPatch) -> AsyncIterator[None]:
    ...
    # The rig IS a transaction pooler, so the fixture declares it -- exactly
    # as a pooled deployment's manifest does. Without this the cache defaults
    # to asyncpg's 100 and the concurrent test fails through the bouncer,
    # which is the whole failure mode this suite exists to pin.
    monkeypatch.setattr(db, "POSTGRES_BEHIND_POOLER", True)
    monkeypatch.setattr(db, "POSTGRES_STATEMENT_CACHE_SIZE", 0)
    monkeypatch.setattr(db, "pool", None)
...
async def test_nested_connection_inside_transaction_fails_fast(
    ...
    The sites that do this are the ones allowlisted in
    NESTED_CONNECTION_LEGACY (scripts/check_crm_boundaries.py, rule 13) --
    that set is the single list, so it cannot drift from this docstring.
    ...
    # The contract is BOUNDED FAILURE. wait_for above proves "not a hang";
    # this proves the other half. Twelve atoms need 2 connections each and
    # TWO ceilings are below that: the app pool (max_size=4 in rig_pool) and
    # PgBouncer's default_pool_size=2. Measured on the rig, relieving one is
    # not enough -- app 4 / bouncer 50 still starves 10 of 12; only app 40 /
    # bouncer 50 lets all 12 through. So this assertion is load-bearing: it
    # fires the moment BOTH bounds stop binding, which is exactly the
    # misconfiguration that would make production hang instead of error.
    failures = [r for r in results if isinstance(r, BaseException)]
    assert failures, (
        "12 nested atoms against default_pool_size=2 must starve; all of "
        "them succeeding means neither POSTGRES_ACQUIRE_TIMEOUT_SECONDS nor "
        "PgBouncer's query_wait_timeout is bounding the wait"
    )
    # And they must fail by a BOUND, not by chance: the app-side acquire
    # timeout, or the bouncer cutting the client off at query_wait_timeout.
    unexpected = [
        r
        for r in failures
        if not isinstance(r, (TimeoutError, PostgresError, InterfaceError))
    ]
    assert not unexpected, f"starvation failed in an unbounded way: {unexpected}"
...
# --- the topology flag drives the cache default -----------------------------
# POSTGRES_BEHIND_POOLER declares what this pool connects into. The cache
# default is derived from it, so a deployment states its topology once instead
# of the app guessing from port numbers.


async def _init_with(
    monkeypatch: pytest.MonkeyPatch, cache: int, behind_pooler: bool
) -> list:
    """Init the pool against a fake asyncpg, returning captured log records."""
    ...
    monkeypatch.setattr(db.asyncpg, "create_pool", fake_create_pool)
    monkeypatch.setattr(db, "decrypt_kms", passthrough)
    for name in ("USER", "PASSWORD", "DB", "HOST", "PORT"):
        monkeypatch.setattr(db, f"POSTGRES_{name}", "x")
    monkeypatch.setattr(db, "POSTGRES_STATEMENT_CACHE_SIZE", cache)
    monkeypatch.setattr(db, "POSTGRES_BEHIND_POOLER", behind_pooler)
    monkeypatch.setattr(db, "pool", None)

    seen: list = []
    monkeypatch.setattr(db.logger, "error", lambda *a, **k: seen.append(("error", a)))
    monkeypatch.setattr(
        db.logger, "warning", lambda *a, **k: seen.append(("warning", a))
    )
    monkeypatch.setattr(db.logger, "info", lambda *a, **k: seen.append(("info", a)))
    await db.init_db_pool(min_size=1, max_size=2)
    return seen


async def test_cache_behind_a_declared_pooler_records_the_coupling(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A cache behind a pooler is VALID, with an unverifiable precondition.

    Behind a pooler the cache defaults to 0, so a non-zero value was set
    deliberately -- and it is correct while the pooler pins
    max_prepared_statements > 0. The app only sees a socket and cannot check
    that, so it records the coupling rather than erroring on a working
    configuration.
    """
    seen = await _init_with(monkeypatch, cache=100, behind_pooler=True)
    assert any(
        level == "warning" for level, _ in seen
    ), f"cache=100 behind a pooler must record the coupling: {seen}"
    assert not any(
        level == "error" for level, _ in seen
    ), f"a deliberate, working configuration must not log an error: {seen}"


async def test_cache_on_a_direct_pool_is_not_an_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A direct deployment keeping the cache is CORRECT -- do not cry wolf.

    Only voice-agent sits behind the bouncer; clairvoyance and the crm workers
    stay direct, and an error on every boot there would bury the real one.
    """
    seen = await _init_with(monkeypatch, cache=100, behind_pooler=False)
    assert not any(
        level == "error" for level, _ in seen
    ), f"cache=100 direct-to-Postgres must not log an error: {seen}"


def test_cache_default_follows_the_declared_topology() -> None:
    """The default is derived, so a pooled deployment cannot forget the 0."""
    import importlib

    import app.core.config.static as static

    for behind, expected in ((True, 0), (False, 100)):
        os.environ["POSTGRES_BEHIND_POOLER"] = "true" if behind else "false"
        os.environ.pop("POSTGRES_STATEMENT_CACHE_SIZE", None)
        importlib.reload(static)
        assert static.POSTGRES_STATEMENT_CACHE_SIZE == expected, (
            f"POSTGRES_BEHIND_POOLER={behind} must default the cache to "
            f"{expected}, got {static.POSTGRES_STATEMENT_CACHE_SIZE}"
        )

    # An explicit value always wins -- the max_prepared_statements escape hatch.
    os.environ["POSTGRES_BEHIND_POOLER"] = "true"
    os.environ["POSTGRES_STATEMENT_CACHE_SIZE"] = "250"
    importlib.reload(static)
    assert static.POSTGRES_STATEMENT_CACHE_SIZE == 250

    os.environ.pop("POSTGRES_BEHIND_POOLER", None)
    os.environ.pop("POSTGRES_STATEMENT_CACHE_SIZE", None)
    importlib.reload(static)
