...
async def test_a_transient_dns_failure_is_retried_not_reported_as_a_refusal(
    ...
    validate_egress_url turns socket.gaierror into an egress error, and this
    branch aborts on SSRFError without retrying. Collapsing the two would end
    the request on the first attempt and tell the model that policy blocked it
    — where before the guard existed, a DNS failure surfaced as an
    aiohttp.ClientConnectorError and was retried like any other transport blip.
    """
    import socket
    ...
    blob = "\n".join(logged)
    assert "could not resolve host" in blob
    assert "blocked by egress guard" not in blob
