...
def _make_setup() -> FrameProcessorSetup:
    """pipecat 1.8 sets strategies up with a FrameProcessorSetup, not a bare
    TaskManager. These strategies only use the task manager, so there is no
    PipelineWorker here."""
    return FrameProcessorSetup(
        clock=SystemClock(),
        task_manager=TaskManager(),
        pipeline_worker=None,  # pyrefly: ignore
    )
