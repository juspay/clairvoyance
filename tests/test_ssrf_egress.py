...
def _force_private_egress_blocked(monkeypatch):
    """Pin the escape hatch off, else a local SSRF_ALLOW_PRIVATE_EGRESS=true
    would invert every assertion here."""
...
async def test_validate_egress_url_ip_literal_private_short_circuits(monkeypatch):
    # Regression: a blocked IP literal must not fall through to getaddrinfo.
    def _fail(*a, **k):
...
async def test_validate_egress_url_blocks_dns_name_resolving_to_internal(monkeypatch):
    # The exact bypass PT-07 describes: a name resolving to an internal address.
...
async def test_ssrf_safe_request_blocks_redirect_to_metadata():
    # aiohttp's own redirect following would skip the check; this helper re-validates.
    ...
    assert len(session.requests) == 1  # metadata hop rejected before it was sent


async def test_ssrf_safe_request_strips_auth_on_offsite_redirect():
    # Leaving the allow-list must drop credentials, else a 302 exfiltrates them.
    ...
    # First hop (on the allow-list) kept auth; the off-allow-list hop stripped it.
...
async def test_rewrite_statuses_turn_post_into_a_bodyless_get(status):
    """301/302/303 must not replay the payload, even on the caller's own host.

    The hop here is SAME-origin on purpose: a cross-origin hop drops the body
    anyway, which hid whether the status-code rule worked at all.
    """
    session = _FakeSession(
        [_FakeResp(status, {"Location": "https://8.8.8.8/next"}), _FakeResp(200, {})]
    )
    async with ssrf_safe_request(
        cast(aiohttp.ClientSession, session),
        "POST",
...
async def test_307_preserves_method_and_body_on_the_callers_own_host():
    """307/308 keep method and body — but only where the payload may travel.

    Stays on the host the caller addressed; a 307 to a different host drops
    the body instead, see
    test_ssrf_credential_scope.py::test_a_307_does_not_replay_the_body_to_another_host.
    """
    session = _FakeSession(
        [_FakeResp(307, {"Location": "https://8.8.8.8/next"}), _FakeResp(200, {})]
    )
    async with ssrf_safe_request(
        cast(aiohttp.ClientSession, session),
        "POST",
