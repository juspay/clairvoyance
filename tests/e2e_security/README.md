...
`test_telephony_webhooks_e2e.py` launches the **real application** with Uvicorn
on loopback and drives the **real mounted routes** over real HTTP, with provider
signatures produced by the providers' own SDKs. Unit tests build Starlette
requests in process; only this suite proves the route wiring behaves when a real
client talks to a real server.

`test_ssrf_egress_e2e.py` does not launch the application. It calls the egress
guard helpers directly against a local loopback server, so it proves no packet
leaves for a blocked destination, not that any route is wired to the guard.
