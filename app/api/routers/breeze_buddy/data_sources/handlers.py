...
async def preview_handler(
    ...
    # Fall back to the requested columns when the preview returns no rows, so
    # the caller still sees what was asked for instead of an empty list.
    col_names = list(rows[0].keys()) if rows else (columns or [])
