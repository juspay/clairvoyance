...
def validate_config_access(
    ...
    if shop_identifier and shop_identifier not in current_user.shop_identifiers and "*" not in current_user.shop_identifiers:
        logger.warning(
            f"User {current_user.username} attempted to {operation} configuration "
...
def filter_configs_by_rbac(
    ...
    for config in configs:
        ...
        has_merchant_access = (
            ...
            config.merchant_id in current_user.merchant_ids
        )

        # Check shop access
        has_shop_access = (
            "*" in current_user.shop_identifiers or
            config.shop_identifier in current_user.shop_identifiers
        )
