...
from app.database.accessor.breeze_buddy.credentials import _merge_credential_value
...
async def list_credentials_handler(
    ...
    for that service — the console's account picker. Filtered here, after the
    tenant read, so the scope rules stay in one place."""
    logger.info(
...
async def update_credential_handler(
    ...
    try:
        ...
        if provider and req.value is not None:
            ...
            merged = _merge_credential_value(req.value, existing.value or {})
