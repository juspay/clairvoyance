...
async def list_template_versions_handler(
    ...
    versions, total = await list_template_versions(
        template_id,
...
async def get_template_version_handler(
    ...
    template = await _load_and_authorize(
        template_id, current_user, "read template version"
    )
    found = await get_template_version(
        template_id, version, template.reseller_id, template.merchant_id
    )
    if found is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def rollback_template_handler(
    ...
    # without this a rolled-back Gemini Live template serves inbound/web
    # calls no opening line until its next save. Unmasked head: generation
    # needs the real config. No-op when the restored template doesn't qualify.
    spawn_realtime_opening_line_regeneration(new_head)

    # The audit notice every head change owes the channel — a rollback is a
    # new version, and skipping it would show v3 -> v4, then v5 -> v6, with
    # nothing for v5. The UPDATE bumps current_version in place, so the head
    # it replaced is exactly one below.
    spawn_background_task(
        emit_template_updated_alert(
            template_id=str(new_head.id),
            template_name=new_head.name,
            from_version=new_head.current_version - 1,
            to_version=new_head.current_version,
            updated_by=current_user.username,
            updated_at=(new_head.updated_at or datetime.now(timezone.utc)).isoformat(
                timespec="seconds"
            ),
            merchant_id=new_head.merchant_id,
            change_source=f"rollback to v{body.version}",
        ),
        name=f"template-updated-alert:{new_head.id}",
    )
