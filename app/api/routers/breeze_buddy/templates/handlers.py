...
async def delete_template_handler(
    ...
    - Template must not have active leads (BACKLOG, RETRY, IN_PROGRESS) in lead_call_tracker
    ...
    try:
        ...
        # Safety check: verify template is not in use
        usage = await check_template_usage(template_id)

        blockers = []
        config_count = usage.get("call_execution_config", 0)
        active_leads_count = usage.get("lead_call_tracker", 0)
        ...
        if active_leads_count > 0:
            blockers.append(
                f"{active_leads_count} active lead(s) (BACKLOG/RETRY/IN_PROGRESS) are using this template"
            )
