...
from app.ai.voice.agents.breeze_buddy.services.conversation_analysis.extractor import (
...
def _evaluation_catalog_response(
    ...
    return TopicCatalogResponse(
        ...
        accepted_enabled=bool(config and config.get("enabled")),
        rejected_enabled=False,
        accepted_topics=list(config.get("topics") or []) if config else [],
        rejected_topics=[],
    )
