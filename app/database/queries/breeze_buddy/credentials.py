...
def _named_by_a_template(param: int) -> str:
    """A template names a provider account by credential_id inside its
    configurations jsonb — a reference no FK can see. This is the in-use
    guard, exact (jsonb_path_exists, never LIKE over text: an uppercase or
    hyphen-less spelling, or the id inside prompt text, must not fool it)
    and ATOMIC — the same statement that deletes or strands the row asks it,
    so no window exists between a check and the write. ``param`` is the
    placeholder that carries the id (``$N::text``, the uuid's one canonical
    spelling)."""
    return f"""
    EXISTS (
        ...
        WHERE "configurations" IS NOT NULL
          AND jsonb_path_exists(
                "configurations",
                '$.** ? (@.credential_id == $id)',
                jsonb_build_object('id', ${param}::text)
              )
    )
"""
...
def get_credentials_by_merchant_query(
    ...
    ``provider``: None reads the PLACEHOLDER rows only (`provider IS NULL` —
    the template variables, the hook context, the plain listing), so a
    provider account's key never surfaces as a `{name}`; a provider word
    reads only that provider's accounts — the picker's question.
    ...
    def which(n: int) -> str:
        return '"provider" IS NULL' if provider is None else f'"provider" = ${n}'
    ...
    if reseller_id and merchant_id:
        ...
        """
        return text, [reseller_id, merchant_id] + ([provider] if provider else [])
    if reseller_id:
        text = f"""
            SELECT * FROM "{CREDENTIALS_TABLE}"
        ...
        """
        return text, [reseller_id] + ([provider] if provider else [])
    else:
        text = f"""
            SELECT * FROM "{CREDENTIALS_TABLE}"
            ...
            ORDER BY "name" ASC;
        """
        return text, [provider] if provider else []
...
def update_credential_query(
    ...
    if unless_named_by_a_template:
        guard = f" AND NOT {_named_by_a_template(param_count)}"
...
def delete_credential_query(credential_id: str) -> Tuple[str, List[Any]]:
    ...
    text = f"""
        ...
        WHERE "id" = $1 AND NOT {_named_by_a_template(1)}
        RETURNING *;
    """
    return text, [credential_id]
