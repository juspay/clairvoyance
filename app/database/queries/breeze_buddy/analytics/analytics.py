...
def get_attempts_to_connect_query(filters: Dict[str, Any]) -> Tuple[str, List[Any]]:
    ...
    (aborted leads were cancelled and INVALID_PHONE ones were never dialed at
    all — often with zero attempts — so neither may read as a first-attempt pick).
    """
    conditions, values = build_analytics_where_clause(filters)
    where_clause = " WHERE " + " AND ".join(conditions) if conditions else ""
...
def get_analytics_telephony_numbers_query(
    ...
    """
    Generate query for telephony numbers analytics.
    """
    conditions, values = build_analytics_where_clause(filters)
    ...
    # Add the NOT NULL condition for telephony number
    if where_clause:
