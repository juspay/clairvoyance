...
def _get_sheets_session() -> Optional[AuthorizedSession]:
    ...
    try:
        ...
        # Transient build failure (e.g. KMS/network blip): do NOT latch
        # _session_ok=False — leave it unset so the next call retries instead
        # of disabling Sheets for the whole pod lifetime. (Config errors above
        # — missing/invalid creds — stay latched since they need a restart.)
...
async def fetch_sheet_data(
    ...
    def _fetch():
        ...
        try:
            ...
            # Escape single quotes per A1 notation (a quote inside a quoted
            # sheet name must be doubled), else tab names like "O'Brien" break.
            safe_tab = tab.replace("'", "''")
            range_str = f"'{safe_tab}'!A1:ZZ{max_rows + 1}"
            result = _get_json(
                session,
                "https://sheets.googleapis.com/v4/spreadsheets/"
                f"{spreadsheet_id}/values/{quote(range_str, safe='')}",
