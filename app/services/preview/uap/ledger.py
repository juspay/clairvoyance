...
``app.crm.preview.uap`` — this module gathers the usage and applies the writes.
"""
...
from app.services.preview.uap.accessor import (
    get_agent_usage,
    get_session_draws,
    settle_session_draw,
    upsert_session_draw,
...
async def settle_draw(
    ...
    *,
    ...
    # and write each element back by its own order_id: a journey can hold
    # several draws (a retry after a failed one) and they settle apart.
    before = await get_session_draws(session_id)
    mine = [d for d in before if d.get("journey_id") == journey_id]
    if not mine:
        return []
    after: List[SessionDraw] = []
    draws: List[Dict[str, Any]] = before
    for d in mine:
        order_id = str(d.get("order_id") or "")
        if not order_id:
            continue  # a draw without an order never reached Juspay; nothing to settle
        new_status = settled_status(str(d.get("status") or ""), ny)
        patch = {"ny_payment_status": ny, "updated_at": stamp, "status": new_status}
        draws = await settle_session_draw(session_id, order_id, patch)
