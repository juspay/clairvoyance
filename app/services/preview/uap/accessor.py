...
async def get_session_draws(session_id: str) -> List[Dict[str, Any]]:
    """Every draw on this session, as stored; [] for an unknown session."""
    query, values = session_draws_query(session_id)
    rows = await run_parameterized_query(query, values)
    return _draws(rows[0]["draws"]) if rows else []
...
async def settle_session_draw(
    ...
    """Merge what a NY poll learned into the draw with ``order_id``;
    returns the session's draws afterwards."""
    query, values = settle_session_draw_query(session_id, order_id, patch)
    rows = await run_parameterized_query(query, values)
    return _draws(rows[0]["draws"]) if rows else []
