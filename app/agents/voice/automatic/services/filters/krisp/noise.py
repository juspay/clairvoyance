...
class NoiseFilterFromKrisp(BaseAudioFilter):
    ...
    async def start(self, sample_rate: int):
        if self._krisp_functional:
            ...
            self._carry = np.array([], dtype=np.int16)  # Clear buffer on start
        else:
    ...
    async def stop(self):
        self._session = None
        self._carry = np.array([], dtype=np.int16)  # Clear buffer on stop
    ...
    async def filter(self, audio: bytes) -> bytes:
        ...
        num_complete_frames = samples.size // self._samples_per_frame

        if num_complete_frames == 0:
            # Not enough samples for even one frame - buffer and return empty
            self._carry = samples
            return b""

        # Split into complete frames and remainder
        num_samples_to_process = num_complete_frames * self._samples_per_frame
        frames = samples[:num_samples_to_process].reshape(-1, self._samples_per_frame)
        self._carry = samples[num_samples_to_process:]  # Save remainder for next call

        # Process complete frames
        processed_samples = np.empty(num_samples_to_process, dtype=np.int16)
