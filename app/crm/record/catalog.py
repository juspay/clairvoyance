...
# Computed on read (ops) or code-layer only (fallbacks, derived): never
# stored in a T24 row — canon's field list is what the row carries.
_CODE_ONLY_KEYS = {"ops", "fallbacks", "derived"}


# --- The CODE layer ---------------------------------------------------------
#
# One entry per (source, topic), DECLARED beside its derivers in the source's
# spec module (one source, one file) and assembled through
# extractors.SPEC_MODULES — this file never names a source. Adding an event
# = one entry + a recorded fixture, same PR, pinned by tests/crm/test_catalog.py;
# adding a source = one import + one registry line in extractors/__init__.py.
...
DERIVE: Dict[Tuple[str, str], Dict[str, Deriver]] = {
    (entry.source, entry.topic): {
        f.path: module.DERIVERS[f.path]
        for f in entry.fields
        if f.derived and f.path in module.DERIVERS
    }
    for module in SPEC_MODULES
    for entry in module.ENTRIES
}
...
def validate_registration(registration: SchemaRegistration) -> List[str]:
    ...
    problems: List[str] = []
    if (registration.source, registration.topic) in CATALOG:
        # The code layer is authoritative for its own events: a shadow row
        # would be gathered by the validator (same source, so the ambiguity
        # guard is silent), hidden by the console and ignored by decode —
        # a plan could then template on a field no send can ever fill.
        # Discovery writes no nudge for a code-layer topic; registration
        # keeps the same rule. A merchant's own vendor picks its own source.
        problems.append(
            f"{registration.source}/{registration.topic} is declared in code — "
            "nothing to register"
        )
...
def dropped_paths(previous: EventSchema, registration: SchemaRegistration) -> List[str]:
    """PURE decide: additive-or-deprecate (design/event-catalog.md §Laws —
    "fields are deprecated, never deleted"). A re-registration that drops
    a path a live flow may filter or template on is refused; the vendor
    keeps the path and marks it `deprecated: true` instead."""
    kept = {f.path for f in registration.fields}
    return [
        f"{f.path}: declared in v{previous.version} and missing now — "
        "a field is deprecated, never deleted (keep it with deprecated: true)"
...
async def merged_entries(merchant_id: str) -> List[CatalogEntry]:
    """Code layer + this merchant's registered layer, one shape, counts
    NOT filled (seen_7d = 0): everything the ETag hashes, and nothing it
    excludes — so a 304 costs one schema read and no GROUP BY."""
    rows = await accessor.list_schemas(merchant_id)
    entries = list(code_entries())
    code_keys = set(CATALOG)
    for row in rows:
        if (row.source, row.topic) in code_keys:
            continue  # the code layer is authoritative for its own events
        entries.append(_entry_from_row(row, 0))
    return entries


async def with_seen_counts(
    merchant_id: str, entries: List[CatalogEntry]
) -> List[CatalogEntry]:
    """The seen-this-week count, computed on read (T24 stays cold) —
    after the ETag check, only for a body that is actually sent."""
    counts = {
...
async def merged_catalog(merchant_id: str) -> List[CatalogEntry]:
    """Code layer + this merchant's registered layer, with the seen-this-week
    count filled: what a caller with no ETag to offer reads."""
    return await with_seen_counts(merchant_id, await merged_entries(merchant_id))
...
async def catalog_fields(
    ...
    """The validator's gather: every declared field for a topic (both
    layers), by path. None = no layer knows this topic.

    Catalog identity is (source, topic) and a door carries topic only
    (canon T19 — `entry.source` is a ruling owed), so a topic that TWO
    sources declare for this merchant cannot be honestly gathered: a
    union would let a condition on a path only one source has match
    nothing for the other, silently. It raises AmbiguousTopic naming
    both; the validator turns that into a problem."""
    ...
    if len(sources) > 1:
        raise AmbiguousTopic(topic, sources)
    return found if sources else None


async def register_schema(
    merchant_id: str, registration: SchemaRegistration, registered_by: str
) -> EventSchema:
    """gather (the previous version) -> decide (the laws, incl. the
    additive-or-deprecate law against it) -> apply (the row)."""
    problems = validate_registration(registration)
    previous = await accessor.get_schema(
        merchant_id, registration.source, registration.topic
    )
    if previous is not None and previous.status == "registered":
        problems.extend(dropped_paths(previous, registration))
    if problems:
...
class AmbiguousTopic(Exception):
    """A topic two sources declare for one merchant: no honest field map
    exists for a door that names the topic alone."""

    def __init__(self, topic: str, sources: List[str]):
        self.topic = topic
