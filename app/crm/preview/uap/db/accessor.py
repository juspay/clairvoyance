...
from app.crm.preview.uap.db.decoder import decode_agent_entry
...
def choose_preferred(entries: Entries, agent_obj_ref: str) -> Tuple[Entries, bool]:
    ...
    so at most one attempt per rider is ever preferred. Only a drawable
    attempt can be chosen: preferring a pending or dead one would make the
    booking path skip every working agent. Nothing changes on a refusal."""
    chosen = find_by_ref(entries, agent_obj_ref)
    if chosen is None or not _is_drawable(chosen):
...
async def insert_attempt(
    *,
    ...
    # tenant-pinned: identity refuses another merchant's customer under the
    # lock, before anything is written — an attempt can never land on a
    # customer of a different merchant.
    result = await mutate_customer_attribute(
        customer_id, AGENTS_KEY, mutation, merchant_id=merchant_id
    )
    if result is None:
        raise RuntimeError(
            f"agentic: customer {customer_id} not found for {merchant_id}"
        )
