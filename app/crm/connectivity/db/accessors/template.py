...
async def stale_submit_claims(
    merchant_id: str, channel: str, name: str, language: str
) -> List[TemplateRead]:
    """Crashed submit claims under this natural key, across accounts. Each
    row carries the account holding it."""
    query, values = stale_submit_claims_query(merchant_id, channel, name, language)
    async with crm_connection() as conn:
        rows = await conn.fetch(query, *values)
    return [decode_template(row) for row in rows]
