...
async def consume_template_event(
    ...
    if applied is None:
        # A guarded write declined, and the log must not name one reason as
        # if it were the only one: the row is retired, or it already carries
        # a newer state, or it moved while this letter was in flight, or the
        # provider's face returned a state with nothing in it for this
        # topic. All four are the guards doing their job rather than errors
        # — and a provider that reorders often is a fact worth seeing.
        logger.info(
            f"template events: {event.topic} for template {template.id} was "
            f"not applied — the row is retired, newer, or moved "
            f"(event {event.id})"
        )
        return
    logger.info(
...
async def _resume_crashed_submit(
    ...
    Order matters. The account-free probe runs FIRST because "this id names
    no local row" is an ordinary, permanent condition — a template managed
    in the provider's own console produces one such letter on every change,
    forever — and asking the cheap question first means that case costs one
    indexed read instead of an installations lookup and a warning about a
    repair nobody needed. Only when a crashed claim actually exists is it
    worth working out which account the letter arrived through.

    What the resume CANNOT recover is ``submitted_category``: the category
    we ASKED for lived only in the request whose process died. The row comes
    back with the provider's own category once their next category letter
    lands, and the 061-choice-4 comparison is simply unavailable for a
    template whose submission crashed — an honest gap, not one to fill with
    a guess.
    """
    provider_template_id = state.provider_template_id
    if event.topic != TOPIC_TEMPLATE_STATUS or not state.status:
        return
    if not (provider_template_id and state.name and state.language and spec.channel):
        return

    claims = await template_accessor.stale_submit_claims(
        event.merchant_id, spec.channel, state.name, state.language
    )
    if not claims:
        # Nothing crashed under that name: the template is managed outside
        # this registry, already retired, or its submit is still in flight
        # (the probe's age predicate excludes a healthy one).
        return

    account = await _account_the_letter_arrived_through(event, spec)
    if account is None:
        return
    claimed = next((c for c in claims if c.provider_account_ref == account), None)
    if claimed is None:
        logger.warning(
            f"template events: '{state.name}' ({state.language}) has a crashed "
            f"submit, but on an account other than the one this letter "
            f"arrived through ({account}) — not resuming (event {event.id})"
        )
        return
...
async def _account_the_letter_arrived_through(
    ...
    """
    accounts = await installation_accessor.connector_accounts(
