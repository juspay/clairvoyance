...
async def execute(
    ...
    contract — is paid only when a rule names customer.*; a customer with
    no row makes those rules not hold (else), while a read that FAILS
    propagates and parks the run for retry — a blip never picks an arm.
