...
# A name here is a name a plan may write; anything else is refused at publish,
# the way customer.<column> is — the whole reason this is a registry and not
# an injection at a call site. Each takes the run's context and the plan's
# exits; anything needing more belongs to its own concern file (ceiling.py).
...
class RunLens(NamedTuple):
    """What a derived run fact is computed FROM. A pair rather than the run
    itself, so this file never learns the shape of a row — the same reason
    `customer` arrives as identity's CustomerFacts and not as a table."""
...
def field_problems(field: str, node_ids: Iterable[str]) -> List[str]:
    ...
    if m.group("derived") is not None and m.group("derived") not in RUN_FACTS:
        ...
        # unbounded, so a misspelt one must be allowed. An ENGINE fact is a
        # closed list, so a typo is a sentence the author reads here instead
        # of a run that takes `else` for the life of the plan.
        return [
...
def lookup(
    ...
    if m.group("derived") is not None:
        # Absent without the lens, the customer arm's rule: unreadable, not
        # False — False would be an answer.
...
def needs_run(rules: Iterable[ConditionRule]) -> bool:
    ...
    supply the lens should know before it judges."""
