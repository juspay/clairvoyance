...
def alarm(node: WorkflowNode, now: datetime, run_ends_at: datetime) -> datetime:
    ...
    if node.minutes:
        ...
        if node.window and in_reserved_period(now, node.window):
            ...
            # ends past the hours.
            minutes += node.window.held_runs_first_minutes
...
def in_reserved_period(at: datetime, window: WaitWindow) -> bool:
    ...
    opening to opening + held_runs_first_minutes, on the window's clock? A window
    with no offset has none. `at` must be aware."""
    if not window.held_runs_first_minutes:
        return False
    if at.tzinfo is None:
    ...
    return (
        opening <= local < opening + timedelta(minutes=window.held_runs_first_minutes)
    )
