...
def validate_definition(
    ...
    for src, arrows in definition.outgoing().items():
        ...
        if node is not None and awaits(node):
            # An awaiting call (21 Sep 2026) takes its ONE plain edge when
            # its report lands or its backstop fires, and a labelled arrow
            # when a merchant topic it lists arrives — the customer acted,
            # so the plan re-decides. Labels name listed topics, or `else`.
            if labels.count(None) != 1:
                problems.append(
                    f"call {src} awaits its report, so it has exactly one plain "
                    "edge (taken when the call ends) — labelled arrows are for "
                    "the topics it lists"
                )
            for on in labels:
