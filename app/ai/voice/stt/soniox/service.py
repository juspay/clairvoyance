...
- **Tighter websocket liveness** — pipecat leaves the websockets library's ping
  defaults in place (ping every 20s, 20s pong timeout), so a silently dead
  connection takes 20-40s to surface (the incident gaps sat exactly in that
  window). 3s/5s cuts detection to <=8s worst case, after which the base
  class's automatic reconnect takes over. The close timeout is pipecat's own
  ``ws_close_timeout`` (2s by default since 1.8), passed through by the
  builder.
...
pipecat 1.8 made ``max_endpoint_delay_ms`` a real setting and added
``ws_close_timeout``, so both left this override; what remains is the watchdog,
the ping cadence, the disconnect flush, and the logging.
"""
...
class SonioxSTTServiceWithEndpointDelay(SonioxSTTService):
    """SonioxSTTService with an endpoint watchdog that force-finalizes stale
    un-endpointed transcripts.
    ...
    detection on, the threshold is floored at ``2 *
    settings.max_endpoint_delay_ms`` (min 1.0s) so it can never preempt a
    healthy semantic endpoint.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # NOT_GIVEN when unset, so isinstance rather than a truth test.
        endpoint_delay_ms = self._settings.max_endpoint_delay_ms
        if not isinstance(endpoint_delay_ms, int):
            endpoint_delay_ms = None
        if (
            not self._vad_force_turn_endpoint
            and endpoint_delay_ms
            and self._finalize_after_secs
        ):
            floor = max(2 * endpoint_delay_ms / 1000, 1.0)
    ...
    async def _websocket_connect(self, uri: str, **kwargs):
        """Open the session with a ping cadence tight enough to notice a dead
        path. pipecat sets ``close_timeout`` from ``ws_close_timeout``; the
        library's own 20s/20s ping defaults are what leave a silently dead
        connection undetected for 20-40s."""
        if self._ws_ping_interval is not None:
            kwargs.setdefault("ping_interval", self._ws_ping_interval)
        if self._ws_ping_timeout is not None:
            kwargs.setdefault("ping_timeout", self._ws_ping_timeout)
        return await super()._websocket_connect(uri, **kwargs)

    async def _connect_websocket(self):
        """Arm the watchdog for the session pipecat just opened.

        Reconnects go through _disconnect_websocket + _connect_websocket, so
        spawning here (idempotently) covers the initial connect and every
        automatic reconnect.
        """
    ...
    async def _flush_buffered_final_transcript(self) -> None:
        ...
        try:
            # Usage before the frame, so tracing attaches it to the STT span
            # the frame closes — same order as pipecat's own endpoint flush.
            ...
            # Soniox turn-detection mode leaves the user turn open until the
            # endpoint closes it; the endpoint never came, so close it here.
