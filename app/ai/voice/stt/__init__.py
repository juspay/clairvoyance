...
def parse_soniox_context(
    ...
    Args:
        context_json: JSON string containing context. If None, uses SONIOX_CONTEXT.

    Returns:
        SonioxContextObject if parsing succeeds, None otherwise
    """
    context_source = context_json or SONIOX_CONTEXT
    if not context_source:
        return None

    try:
        # Parse JSON from environment variable
        context_data = json.loads(context_source)
        ...
        logger.warning(
            f"Failed to parse Soniox context: {e}. Falling back to None context."
        )
        return None


async def get_stt_service(
    voice_name: Optional[str] = None, agent_type: str = "automatic"
):
    """
    Returns an STT service instance based on the environment configuration.
    Supports both Automatic and Breeze Buddy agents.

    Args:
        voice_name: Voice name to determine STT provider override for specific voices
        agent_type: Type of agent ("automatic" or "breeze_buddy")
    """
    # ============================================================================
    # BREEZE BUDDY AGENT STT CONFIGURATION
    # ============================================================================
    if agent_type == "breeze_buddy":
        if BREEZE_BUDDY_STT_SERVICE == "openai":
            ...
            if BREEZE_BUDDY_SONIOX_LANGUAGE_HINTS:
                lang_list = [
                    lang.strip()
                    for lang in BREEZE_BUDDY_SONIOX_LANGUAGE_HINTS.split(",")
                ]
                language_hints = [Language(lang) for lang in lang_list if lang]

            # Parse context from Breeze Buddy specific JSON environment variable
            context = parse_soniox_context(BREEZE_BUDDY_SONIOX_CONTEXT)
            ...
            # Default to Google STT for Breeze Buddy
            logger.info(
                "Using Google STT service for Breeze Buddy with VAD-based turn detection"
            )
    ...
    # ============================================================================
    # AUTOMATIC AGENT STT CONFIGURATION
    # ============================================================================
