...
exact tools array + one tiny synthetic user turn, fire-and-forget, in the gap
between flow init and the first inference — for standard outbound telephony
that gap is the greeting playback (~3-6s), which is the whole point: turn 1
then reads a warm cache.
...
handling, and the ``no tools ⇒ no tools key`` rule cannot drift. The
synthetic user turn sits AFTER the warmed prefix — strict gateways
(breeze/sglang) 400 prefix-only requests with "No user query found in
messages." — so turn-1's real utterance changes only tokens the cache never
needs to match (live-proven: warm → turn-1 ``cached_tokens`` == the shared
prefix length). The failure mode of any mismatch is only a cache miss
...
pipecat-ai 1.1.0 + pipecat-ai-flows 1.0.0; revisit on upgrade.
"""
...
# Strict gateways (breeze/sglang) require a user turn in every request and
# 400 prefix-only bodies ("No user query found in messages."). The content
# is irrelevant to the cache — it sits AFTER the warmed prefix.
_PREFILL_SYNTHETIC_USER_TURN = "."
    ...
    try:
        ...
        return spawn_background_task(
            prefill_system_prompt(
                ...
                messages=messages,
                # Evaluated in THIS frame — a bad function entry can raise
                # here, outside prefill_system_prompt's try. The except below
                # keeps the never-raises contract: this runs before
                # flow_manager.initialize, and a raise here would kill flow
                # init (greeting, then silence).
        ...
        logger.opt(exception=exc).warning(
            "prefill: spawn failed — call proceeds on a cold cache"
        )
        return None
...
async def prefill_system_prompt(
    *,
    ...
    never re-raised: a broken prefill must not be able to affect the call it
    is warming. Deliberately NOT tracked into the call's errors list — that
    list means "the call degraded"; a cold cache did not degrade anything,
    and end_conversation snapshots the list nondeterministically relative to
    this background task anyway. The WARNING below carries the traceback and
    is greppable as ``prefill: failed``.

    ``messages`` is role_messages + task_messages in pipecat-flows'
    serialization order — the longest common prefix of both first-request
    shapes (greeting played → ``+ user transcript``; respond_immediately →
    exactly this list). The REQUEST appends one tiny synthetic user turn
    AFTER that prefix (strict gateways 400 prefix-only bodies); turn-1's
    real utterance replaces it, so the warmed prefix is untouched.
    """
