...
the chat driver does (pipecat 1.8 exposes no public non-streaming entry point
that returns usage — ``run_inference`` discards it). Verified against
pipecat 1.8.1 (flows bundled as ``pipecat.flows``); revisit on upgrade.
"""
...
def spawn_prefill(
    *,
    ...
    initial_node_config: Any,  # pipecat.flows NodeConfig (a TypedDict)
...
async def prefill_system_prompt(
    *,
    ...
    try:
        ...
        # Same conversion pipecat.flows' adapter.format_functions performs
        # (to_function_schema → ToolsSchema; empty → NOT_GIVEN, which the LLM
        # adapter and the OpenAI SDK both collapse to "no tools key") —
        # format_functions itself lives on the flows adapter, not the
...
def _function_entries(
    ...
    """Mirror pipecat.flows FlowManager._set_node's function registration
    (globals FIRST, then node functions; callables wrap as
    FlowsDirectFunctionWrapper, FlowsFunctionSchema pass through). Anything
