...
async def build_turn_strategies_from_template(
    ...
    if not turn_strategy_config:
        logger.debug("No turn strategies configured, using pipecat defaults")
        return None, 5.0

    # Convert dict to TurnStrategyConfigModel if needed
    if isinstance(turn_strategy_config, dict):
        try:
            turn_strategy_config = TurnStrategyConfigModel(**turn_strategy_config)
        except Exception as e:
            logger.warning(f"Invalid turn_strategy_config: {e}, using defaults")
            return None, 5.0

    # If enabled, build the strategies
    if turn_strategy_config.enabled:
    ...
    # Not enabled
    logger.debug("Turn strategies disabled in config, using pipecat defaults")
    return None, 5.0


async def build_turn_strategies_from_config(
    config: Union[TurnStrategyConfigModel, Dict[str, Any]],
) -> Tuple[Optional[UserTurnStrategies], float]:
    ...
    if isinstance(config, dict):
        config = TurnStrategyConfigModel(**config)
