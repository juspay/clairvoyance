...
def _gate_flow_functions(flow: Dict[str, Any]) -> List[FlowFunction]:
    """Resolve the gate's action set from the template flow: the handler-bearing
    functions of the initial node (node-mode) or the flat functions array
    (direct-mode) — the same source the gate uses for its system prompt.

    Raw template ``nodes`` is a LIST of FlowNodeModel dicts (keyed by
    ``node_name``); ``flow_config`` re-keys them by name later, but we read the
    raw flow here. The legacy ``function_name`` key is normalized to ``name``
    (builder parity). Only handler-bearing functions are gate actions — the gate
    has no node state machine, so transition_to / hooks-only functions (the
    normal node flow) are not callable actions here.
    """

    def _to_flow_fn(func_data: Dict[str, Any]) -> FlowFunction:
        if "name" not in func_data and "function_name" in func_data:
            func_data = {**func_data, "name": func_data["function_name"]}
        return FlowFunction.model_validate(func_data)

    if flow.get("mode") == FlowMode.DIRECT.value:
        raw_fns = flow.get("functions") or []
    else:
        init_name = flow.get("initial_node")
        nodes = flow.get("nodes") or []
        if isinstance(nodes, list):
            # Raw template form: nodes is a list keyed by `node_name`.
            init_node = next(
                (
                    n
                    for n in nodes
                    if isinstance(n, dict)
                    and (n.get("node_name") or n.get("name")) == init_name
                ),
                {},
            )
        elif isinstance(nodes, dict):
            init_node = nodes.get(init_name or "", {}) or {}
        else:
            init_node = {}
        raw_fns = init_node.get("functions") or []

    return [f for f in (_to_flow_fn(x) for x in raw_fns) if f.handler]
...
class Agent:
    ...
    async def _run_generation(self) -> None:
        ...
        if is_scripted:
            ...
            # Gate actions come from the flow itself — the initial node's
            # functions (node-mode) or the flat functions array (direct-mode) —
            # the same source the gate uses for its system prompt. A function's
            # gate-only ``handler``/``line`` are inert in the normal node flow,
            # so one definition serves both paths.
            flow = (self.template.flow if self.template else None) or {}
            scripted_fns = _gate_flow_functions(flow)
