...
class Agent:
    ...
    async def _evaluate_deferred_rate_limit(
        ...
        """
        # Phase 1: Lookup & policy evaluation — fail-open.
        # If template lookup or Redis is down, allow the call through.
        try:
            ...
            # Fail-open: if lookup or policy evaluation fails, allow the call through
            logger.error(
        ...
        # Call is allowed — no enforcement needed
        if policy_result.allowed:
            return False

        # Phase 2: Enforcement — fail-closed.
        # Policy has denied the call. Even if enforcement (transfer/block)
        # throws, the call MUST NOT proceed.
        logger.info(
        ...
        try:
            ...
            # Fail-closed: policy already denied this call. Even if enforcement
            # (transfer/TTS playback) fails, the call must NOT proceed.
            logger.error(
                f"[IVR-RateLimit] Enforcement failed for denied call {self.call_sid}, "
                f"still blocking: {e}",
                exc_info=True,
            )
            # Best-effort: try to close WebSocket
            try:
                if self.ws:
                    await close_websocket_safely(
                        self.ws, code=4000, reason="Rate limit enforcement error"
                    )
            except Exception:
                pass

        # Always return True (blocked) once policy denied the call
        return True
