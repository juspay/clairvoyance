...
async def _create_tokens(
    ...
    room_url: str,
    *,
    bot_params: Optional[DailyMeetingTokenParams],
) -> tuple[str, str]:
    """Mint the user and bot meeting tokens concurrently.

    Worth being explicit about why this is safe to parallelise and why it only
    pays off here. Both calls take room_url and neither reads the other's
    result, so there is no ordering to preserve. But HTTP/1.1 cannot multiplex:
    two concurrent requests need two sockets, so on a cold pool this opens a
    second connection and pays a second TLS handshake. Gathering is a win only
    because the pool holds warm connections (see _pools.py) — measured 786ms
    sequential against 533ms gathered on a warm pool, and a losing 2052ms on a
    cold one.

    gather(return_exceptions=True) rather than a bare gather so a failure in
    one call cannot leave the other running unobserved; the first exception is
    re-raised unchanged, so callers still see the aiohttp/Daily error itself.
    """
    user_token, bot_token = await asyncio.gather(
        call_with_retry(lambda: daily_rest.get_token(room_url), what="get_token(user)"),
        call_with_retry(
            lambda: daily_rest.get_token(room_url, expiry_time=3600, params=bot_params),
            what="get_token(bot)",
        ),
        return_exceptions=True,
    )
    # Checked one at a time, not in a loop: narrowing BaseException away is
    # what lets the return type stay tuple[str, str].
...
async def start_daily_session(
    ...
    # Take a room that was built in advance. Creating one costs three round
    # trips to api.daily.co (~476ms even warm) and none of that work depends on
    # the lead, so it happens before the request arrives. An empty or disabled
    # pool creates the room inline, which is the pre-pool behaviour — see
    # room_pool.py.
    pooled = await acquire_room(enable_recording)
    room_url = pooled.room_url
    room_name = pooled.room_name
    user_token = pooled.user_token
    bot_token = pooled.bot_token
    ...
    updated_lead = await update_lead_call_id_by_id(lead_id, room_name)
    if not updated_lead:
        logger.warning(
        ...
        recording_lead = await update_lead_call_recording_url(room_name, "daily")
        ...
        cleared_lead = await update_lead_call_recording_url(room_name, "")
        if not cleared_lead:
            logger.warning(
    ...
    try:
        ...
        if enable_recording:
            try:
                await update_lead_call_recording_url(room_name, "")
