...
async def download_call_recording(
    ...
    """
    try:
        # The pod-wide connection the room-creation path uses: opening a
        # private session here would pay the 676ms api.daily.co handshake
        # again (see _pools.py). No `async with` — the session outlives us,
        # and closing it would strand every other caller's connection.
