...
class SseRtviForwarder:
    """Callable that records SSE chunks and mirrors each to the Daily client.

    Pass an instance as the ``on_sse_event`` callback to
    ``HttpRequestExecutor.execute()``. After the stream completes, call
    ``emit_end()`` to send a terminal ``global-function-sse-end`` marker.
    """
