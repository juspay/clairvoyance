...
async def resolve(
    ...
    *,
    ...
    waiting. The rendered read gets only what is left after the logo's share,
    so a slow render costs that one source, not the platform's look.
    """
    warnings: List[str] = []
    provider_look: Optional[BrandLook] = None
    provider_timeout = firecrawl.DEFAULT_TIMEOUT_SECONDS
    if deadline is not None:
        provider_timeout = min(
            provider_timeout,
            deadline - time.monotonic() - _LOGO_TIMEOUT_SECONDS,
        )
    try:
        if provider_timeout <= 0:
            raise firecrawl.BrandLookUnavailable("no time left before the deadline")
        provider_look = await firecrawl.brand_look(
            url, timeout_seconds=provider_timeout
        )
