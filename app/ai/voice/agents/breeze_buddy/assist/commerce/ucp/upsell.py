...
**OPT-IN** — off unless the template sets ``flavor.ucp.features.upsell``.
Merchandising is a merchant's call, not a side effect of enabling the
commerce flavor, and the feature spends a real budget per add_to_cart (one
micro-LLM call + one catalog search). A merchant who hasn't opted in pays
nothing: the gate returns before any work.

Once on, it fires as the ``add_to_cart`` policy's ``followup`` after the
CartView is already on the wire, so the recommendation costs the shopper no
perceived latency. No further configuration — one flag is the whole surface:
...
- **What to recommend** — a micro-LLM call (same pooled Gemini service the
  ...
  probed live and rejected (the gateway's ``related`` returns substitutes —
  more leggings after adding leggings; ``complementary`` is empty unless
  the merchant hand-curates pairings), and per-merchant maps don't
...
async def _pick_complement_queries(
    ...
    # The template's own LLM account (accounts), like every
...
async def run_cart_upsell(
    ...
    OPT-IN: returns immediately unless the template sets
    ``flavor.ucp.features.upsell``. The gate is here rather than on the
    policy because policies register once per process while the flag is
    per-template — and it is checked BEFORE any work, so a merchant who
    hasn't opted in pays nothing (no LLM call, no catalog search).
    """
    from app.ai.voice.agents.breeze_buddy.assist.commerce.ucp.intents import (
        resolve_cart_config,
    )

    if not resolve_cart_config(getattr(agent, "template", None)).upsell_enabled:
        return None
    try:
