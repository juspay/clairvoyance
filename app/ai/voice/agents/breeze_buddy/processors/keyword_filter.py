...
class BusyStateKeywordFilter(FrameProcessor):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._active_function_call_count = 0
    ...
    def is_bot_busy(self) -> bool:
        ...
        return (
            self._bot_speaking
            or self._llm_processing
            or self._active_function_call_count > 0
        )
    ...
    def _should_filter_transcription(self, text: str) -> bool:
        ...
        if self._match_mode == "exact":
            # Only filter when every word is a keyword — transcriptions
            # containing keywords *plus* other words are preserved.
            words = normalized_text.split()
            if not words:
                return False
            return all(word in self._normalized_keywords for word in words)
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        ...
        elif FUNCTION_CALL_FRAMES_AVAILABLE and isinstance(
            frame, FunctionCallInProgressFrame
        ):
            self._active_function_call_count += 1
            logger.debug(
                "BusyStateKeywordFilter: Function call started (active=%d)",
                self._active_function_call_count,
            )

        elif FUNCTION_CALL_FRAMES_AVAILABLE and isinstance(
            frame, FunctionCallResultFrame
        ):
            self._active_function_call_count = max(
                0, self._active_function_call_count - 1
            )
            logger.debug(
                "BusyStateKeywordFilter: Function call ended (active=%d)",
                self._active_function_call_count,
            )

        # Filter final transcriptions when bot is busy (skip interim)
            ...
            if should_filter and self.is_bot_busy:
                ...
                logger.debug(
                    "BusyStateKeywordFilter: Filtered transcription "
                    "(bot_speaking=%s, llm_processing=%s, function_calls=%d)",
                    self._bot_speaking,
                    self._llm_processing,
                    self._active_function_call_count,
                )
                ...
                logger.debug(
                    "BusyStateKeywordFilter: Allowing keyword transcription (bot is idle)"
                )
