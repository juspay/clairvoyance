...
class FlowFunction(BaseModel):
    ...
    # Gate-only (constrained_output) fields. The normal node flow ignores these
    # — it drives behavior via transition_to + hooks. The constrained gate has no
    # node state machine, so an action declares its builtin handler directly and
    # an optional closing line to speak first. None defaults keep normal/legacy
    # templates fully inert.
    handler: Optional[str] = Field(
        None,
        description="Gate-only (constrained_output). Builtin handler key "
        "(BUILTIN_HANDLERS) to run at dispatch — e.g. 'end_conversation', "
        "'connect_to_live_agent' (warm transfer), 'connect_to_agent', "
        "'hold_and_consult', 'get_current_time', 'query_knowledge_base', "
        "'update_outcome'. Ignored by the normal node flow.",
    )
    line: Optional[int] = Field(
        None,
        description="Gate-only (constrained_output). 1-based phrase id (into "
        "configurations.phrases) the gate SPEAKS before running the handler — "
        "the declarative closing line, resolved with {placeholder} vars from the "
        "lead payload. null = speak nothing before running. Ignored by the normal "
        "node flow.",
    )
