...
def _feature_variant_entry(entry: Dict[str, Any], variant_id: str) -> Dict[str, Any]:
    ...
    Only fields the variant record actually carries are rewritten (price /
    image); everything stays tool-sourced by construction — an unknown
    ``variant_id`` returns the entry untouched (fail-open, same posture as
...
def _select_list_props(
    ...
    When the component schema declares a ``selection_field`` (ProductGrid:
    ``items`` = ``[{id, feature_variant?}]``) and the op carries one, every
    bound list prop is filtered to the entries whose ``id`` appears in the
    selection — in selection ORDER. This is the deterministic answer to
    fuzzy search ranking: the shopper asks for one product, the tool page
    contains ten, and the match may sit at ANY rank — the model names which
    id(s) to show while every rendered VALUE stays tool-sourced (an id
    absent from the tool payload selects nothing, so the model cannot
    inject items). ``feature_variant`` re-derives that entry's hero from
    the named variant record and stamps ``featured_variant_id`` (variant
    continuity: grid hero → PDP preselect → add_to_cart). No id matching at
    all → the filter is ignored entirely (fail-open: the full tool list
    beats an empty render on a model-mangled id).
    """
    ...
    for key in bound_keys:
        ...
        for sel in selectors:
            ...
            if entry is None:
                continue
            fv = sel.get("feature_variant")
            if isinstance(fv, str) and fv:
                entry = _feature_variant_entry(entry, fv)
