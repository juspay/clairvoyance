...
class Agent:
    ...
    async def run(self, runner_args: Optional[RunnerArguments] = None):
        ...
        if self.transport_type == "daily":
            ...
            # Get lead from runner_args using lead_id
            lead_data = runner_args.body
            lead_id = lead_data.get("lead_id")
            if not lead_id:
                raise ValueError(
                    "lead_id is required in runner_args.body for Daily mode"
                )
            ...
            self.lead = lead
            self.call_sid = (
                lead.call_id or f"daily-{datetime.now().strftime('%Y%m%d%H%M%S')}"
            )
            self.reporting_webhook_url = (
                lead.payload.get("reporting_webhook_url") if lead.payload else None
            )

            # Set call_initiated_time for Daily mode (for call duration calculation)
            call_initiated_time = datetime.now(timezone.utc)
            updated_lead = await update_lead_call_initiated_time_by_id(
            ...
            # Create VAD analyzer for Daily mode and store reference for handlers (mute_stt/unmute_stt)
            ...
            # Create Daily transport with the VAD analyzer we just created
            daily_params = {
                "daily": lambda vad=self.vad_analyzer: DailyParams(
        ...
        # Response state gate prevents double-speaking by interrupting
        # active LLM/TTS when new transcriptions arrive
        ...
        # Insert response gate after stt if enabled
        if response_gate:
