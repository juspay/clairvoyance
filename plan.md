...
| `response_config.should_proceed_field` | string | Yes | JSON field name in response that contains the boolean go/no-go |
| `response_config.reason_field` | string | No | JSON field name containing reason string (for logging) |
| `default_on_failure` | bool | Yes | What to assume if the API call fails/times out. `true` = proceed with call (fail-open), `false` = skip call (fail-closed) |
...
The field names (`can_proceed`, `reason`) are configurable via `response_config.should_proceed_field` and `response_config.reason_field`.
...
### 7. New Lead Status for Pre-Check Failure (Optional Enhancement)

No new status needed. When pre-checks fail:
- For the current cron cycle: the lead stays as `BACKLOG` with its lock released — it will be re-evaluated on the next cron run
- The pre-check result is logged but does not permanently block the lead
- This means if a pre-check is time-dependent (e.g., "don't call during dinner"), the lead will naturally be retried later
...
3. **Lead stays in BACKLOG on pre-check failure**: The lead is not permanently rejected — it will be re-evaluated on the next cron cycle. This handles transient failures and time-dependent checks naturally.

4. **Reuses existing HTTP infrastructure**: `HttpRequestConfig`, `HttpAuthConfig`, `HttpRequestExecutor`, placeholder resolution — all existing patterns are reused.

5. **No new database table**: Pre-checks are a JSONB column on `call_execution_config`, keeping the config hierarchy simple.

6. **Backward compatible**: `pre_checks` defaults to NULL. Existing configs with no pre-checks continue to work unchanged.
