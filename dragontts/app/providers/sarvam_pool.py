            ...
            while True:
                try:
                    raw = await asyncio.wait_for(
                        self.ws.recv(), timeout=self._recv_timeout
                    )
                except asyncio.TimeoutError as e:
                    raise ProviderError(
                        "sarvam stream timed out waiting for audio"
                    ) from e
                try:
                    m = json.loads(raw)
...
class SarvamStreamPool:
    ...
    async def start(self) -> None:
        ...
        logger.info(
            f"Sarvam stream pool warming {self._min_size} socket(s) for model={self._model}"
        )
