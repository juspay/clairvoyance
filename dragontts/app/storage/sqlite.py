...
class SQLiteMetadataStore:
    ...
    def _new_connection(self) -> sqlite3.Connection:
        ...
        )  # ~2MB page cache (was 8MB) -- the mmap + OS page cache carry the real read working set, so keep this small; bounds per-connection RSS x (THREAD_POOL_WORKERS conns) x (--workers processes) under the pod's memory limit
        conn.execute("PRAGMA temp_store = MEMORY")
        conn.execute(
            "PRAGMA mmap_size = 67108864"
        )  # 64MB mmap for reads (was 256MB) -- bounds resident mmap pages under the cgroup limit; hot reads still come from the OS page cache
        return conn
        ...
        def _setup() -> None:
            ...
            try:
                ...
                has_totals = conn.execute(
                    "SELECT COUNT(*) FROM provider_totals"
                ).fetchone()[0]
                if has_totals == 0:
                    conn.execute(
                ...
                has_entries = (
                    conn.execute("SELECT COUNT(*) FROM cache_entries").fetchone()[0] > 0
                )
                words_zero = (
                    conn.execute(
                        "SELECT COALESCE(SUM(total_words), 0) FROM provider_totals"
                    ).fetchone()[0]
                    == 0
                )
        ...
        def _w(conn: sqlite3.Connection) -> None:
            ...
            try:
                ...
                if cur.rowcount == 1:  # newly inserted; a concurrent dup was ignored
                    conn.execute(
                        totals_sql,
                        (record.provider, record.size_bytes, _wc(record.text)),
                    )
                conn.execute("COMMIT")
    ...
    async def replace_with_totals(self, record: CacheRecord) -> None:
        ...
        def _w(conn: sqlite3.Connection) -> None:
            ...
            try:
                prev = conn.execute(
                    "SELECT size_bytes, text FROM cache_entries WHERE key = ?",
                    (record.key,),
                ).fetchone()
                ...
                if prev is None:
                    conn.execute(
                        totals_sql, (record.provider, 1, record.size_bytes, new_wc)
                    )
                else:
                    conn.execute(
                        totals_sql,
    ...
    async def delete(self, key: str) -> bool:
        ...
        def _d(conn: sqlite3.Connection) -> bool:
            ...
            try:
                row = conn.execute(
                    "SELECT provider, size_bytes, text FROM cache_entries WHERE key = ?",
                    (key,),
                ).fetchone()
                if row is None:
                    conn.execute("COMMIT")
    ...
    async def daily_summary(
        ...
        synth_calls, words_served/synthesized, bytes_served, creates, deletes)
        AND a per-provider breakdown from ``metrics_daily_provider``.
        Returns ``{date: {"totals": {...raw sums...}, "by_provider": {prov: {...}}}}``.
        Derived rates (hit_rate, words_from_cache_pct) are computed by the
        caller — this returns raw sums only."""
        clauses: list[str] = []
        args: list = []
        if from_date:
            clauses.append("date >= ?")
        ...
        def _q(conn: sqlite3.Connection) -> dict:
            ...
            for r in conn.execute(
                f"SELECT * FROM metrics_daily{where} ORDER BY date", args
            ):
                d = r["date"]
                ...
                out[d]["totals"] = {
                    ...
                    "synth_calls": r["synth_calls"],
                    "base64_uploads": r["base64_uploads"],
                    "creates": r["creates"],
                    "deletes": r["deletes"],
    ...
    async def latency_summary_daily(
        ...
        def _q(conn: sqlite3.Connection) -> dict:
            ...
            dates = [
                r[0]
                for r in conn.execute(
                    f"SELECT DISTINCT date FROM latency_samples{where} ORDER BY date",
                    args,
                )
            ]
            for d in dates:
    ...
    async def stats(self) -> dict:
        ...
        def _q(conn: sqlite3.Connection) -> dict:
            ...
            by_provider = {
                r[0]: {"entries": r[1], "total_bytes": r[2], "total_words": r[3]}
                for r in rows
            }
            return {
